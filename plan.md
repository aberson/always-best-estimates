...
- **Depends on:** 5
- **Status:** DONE (2026-07-08) — statsmodels>=0.14.6 added (adfuller; lazy import, off the hot path). Anchors: noise → d=0.0; RW → d≥0.35 (honest deviation from "d≈1": at threshold 1e-5 grid d<0.35 needs windows wider than a 2000-bar fold — min_d_search returns the smallest TESTABLE passing d; the d=0-fails half + independent ADF re-verification pin the intent; fixed-width FFD's whole point is stationarity at fractional d<1). merge_asof runs PER SERIES on available_date (per-series release lags handled independently — verified vs a pure-Python point-in-time reconstruction). build_features output: flat ASSET_feature float64 columns (dtype pinned for Step 13); macro-None → no macro columns (degraded-mode compatible); macro columns may carry leading NaN (JEPA must handle).
...
- **Depends on:** 13
- **Status:** DONE (2026-07-08) — `eval/walk_forward.py` scores both models THROUGH the production `WorldModel.forecast` on identical purged folds (`purged_walk_forward_splits` + `validate_no_leakage`, horizon=H); JEPA trained per fold via production `train_jepa` on prices ≤ last_train_date only; both models get PRODUCTION-SHAPED frames — each asset's OWN FULL history (the `pipeline._stage_features` shape; SPY from 1993, not the panel-truncated series) cut to dates ≤ t (poison-tested: full-history length + exact max-date per eval point) — the joint panel serves fold geometry + realized `(t, t+H]` alignment only. Pre-registered MECHANICAL promotion rule (docstring, pre-results): promote iff pooled MSE strictly lower AND pooled coverage within ±5pp of 0.90 (inclusive) AND coverage no worse than EWMA (inclusive) — else EWMA default (wins ties). `EvalReport` + `render_report_markdown` (full JEPAConfig fingerprint; per-fold ensemble version labels; pooled-IC = reporting-only caveat) + CLI (`--quick`/full, exit 0 regardless of winner; CLI e2e-tested against a seeded db). Real full-config run committed at `docs/eval/jepa-vs-ewma-2026-07-08.md` (4596-bar joint panel, 4 splits, 525 eval points, 19.3s): **JEPA promoted** on a THIN margin — pooled MSE 1.389e-3 < EWMA 1.429e-3 (~2.8%), coverage JEPA 0.897 vs EWMA 0.924 (both in band; JEPA closer to 0.90). Interpretation: EWMA's 21-day momentum μ has slight NEGATIVE IC (returns mean-revert; per-asset IC −0.17/−0.12/−0.00), JEPA IC ≈ 0 — the edge is EWMA's anti-momentum bias, not real JEPA skill; parity as plan §1 predicted. Promotion stays a MANUAL operator action (ABE_MODEL toggle); scheduler/api untouched, DEFAULT still EWMA.
